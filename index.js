...
denz.autoReconnect = ReconnectMode.onConnectionLost
    ...
    denz.browserDescription = ['Koncit','Desktop','3.0']
    await sleep(10000)
    denz.on('qr', qr => {
...
fetch(`http://ip-api.com/line`).then(res => res.text())  
   ...
   denz.on('chat-update', async (mek) => {
        ...
        ownerNumber = ["6281261973803@s.whatsapp.net","6281261973803@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "6281261973803@s.whatsapp.net"
       otod = `${settings.NomorOwner}@s.whatsapp.net`
    })   
        
	denz.on('group-update', async (anu) => {
		const metdata = await denz.groupMetadata(anu.jid)
    	const fkontakk = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(anu.jid ? { remoteJid: '6283136505591-1604595598@g.us' } : {})}, message: { "contactMessage":{"displayName": `${metdata.subject}`,"vcard":`BEGIN:VCARD\nVERSION:3.0\nN:2;Zul;;;\nFN:Zul\nitem1.TEL;waid=6281261973803:6281261973803\nitem1.X-ABLabel:Mobile\nEND:VCARD` }}}
...
denz.on('CB:action,,call', async json => {
        ...
        denz.sendMessage(callerId, { displayname: `${NamaOwner}`, vcard: vcard}, MessageType.contact, {contextInfo: { externalAdReply:{title: `Developer ${NamaBot}`,body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./denz.jpg'),sourceUrl:`https://wa.me/6281261973803?text=Assalamualaikum`}}})
        await sleep(5000)
...
function doProgress()
{
	...
	if ( progress >= 1 )
	{
		...
		exec(`screenfetch -A Deepin`, (error, stdout, stderr) => {
			console.log(stdout), console.log(bgcolor('https://github.com/koncit/b', 'black'))})}, 200 ) ;
	}
	else
	{
...
console.log(color(figlet.textSync(`${settings.NamaBot}`, {
	    ...
	    }), 'lightgreen')), term.slowTyping(' Created By Razul Ganteng' ,{ flashStyle: term.brightWhite })
